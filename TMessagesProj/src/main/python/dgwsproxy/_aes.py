...
# 1) pycryptodome — предпочтительно на Android (Chaquopy): быстрый C-AES, без системного libcrypto.
if _BACKEND is None:
    try:
        ...
        class _PcCtrStream:
            ...
            def __init__(self, key: bytes, iv: bytes):
                # tg-ws-proxy использует полный 16-байтный IV как начальный счётчик CTR.
                self._c = _PyCryptoAES.new(
                    key, _PyCryptoAES.MODE_CTR,
                    initial_value=int.from_bytes(iv, "big"), nonce=b"")
        ...
        class Cipher:
            ...
            def encryptor(self) -> "_PcCtrStream":
                return _PcCtrStream(self._key, self._iv)

            decryptor = encryptor  # CTR симметричен
...
# 3) ctypes-обёртка над системным libcrypto — роутеры/embedded (fallback).
if _BACKEND is None:
    import ctypes
    import ctypes.util
