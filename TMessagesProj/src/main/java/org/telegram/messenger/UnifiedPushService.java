    ...
    private static void scheduleRegistrationRetry() {
        AndroidUtilities.runOnUIThread(() -> {
            ...
            registrationRetryRunnable = () -> {
                ...
                PushListenerController.IPushListenerServiceProvider provider =
                        PushListenerController.UnifiedPushListenerServiceProvider.INSTANCE;
            ...
            Utilities.globalQueue.postRunnable(() -> {
                ...
                if (!NTFY_DISTRIBUTOR.equals(savedDistributor)) {
                    if (!TextUtils.isEmpty(SharedConfig.unifiedPushGateway)) {
                        try {
                            token = SharedConfig.unifiedPushGateway
                                    + URLEncoder.encode(endpoint.getUrl(), "UTF-8");
