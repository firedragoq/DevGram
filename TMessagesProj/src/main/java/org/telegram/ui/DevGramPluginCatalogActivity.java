...
public class DevGramPluginCatalogActivity extends BaseFragment {
    ...
    private View createCard(Context context, DevGramPlugins.CatalogEntry e) {
        ...
        icon.setBackground(Theme.createRoundRectDrawable(AndroidUtilities.dp(13),
                Theme.multAlpha(Theme.getColor(Theme.key_featuredStickers_addButton, resourceProvider), 0.12f)));
        ...
        if (!e.version.isEmpty()) sub.append(sub.length() > 0 ? "  ·  v" : "v").append(e.version);
        if (sub.length() > 0) {
